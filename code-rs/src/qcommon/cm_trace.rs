...
fn CM_TestInLeaf(cm: &mut clipMap_t, tw: &mut traceWork_t, leaf: &cLeaf_t) {
    ...
    if !cm_noCurves.as_bool() {
        ...
        for opt_patch in leaf.leaf_surfaces_mut(cm).iter_mut() {
...
fn CM_TestCapsuleInCapsule(cm: &clipMap_t, tw: &mut traceWork_t, model: clipHandle_t) {
    ...
    let symetricSize = [mins - offset, maxs - offset];
...
fn CM_TestBoundingBoxInCapsule(cm: &mut clipMap_t, tw: &mut traceWork_t, model: clipHandle_t) {
    ...
    let offset = vec3_t::mid(mins, maxs);
    let size = [mins - offset, maxs - offset];
    tw.start -= offset;
    tw.end -= offset;

    // replace the bounding box with the capsule
    tw.sphere.use_ = true;
    tw.sphere.radius = fmin(size[1][0], size[1][2]);
    tw.sphere.halfheight = size[1][2];
    tw.sphere.offset = VectorSet(0.0, 0.0, size[1][2] - tw.sphere.radius);
...
fn CM_PositionTest(cm: &mut clipMap_t, tw: &mut traceWork_t) {
    ...
    let mut ll = leafList_t {
        ...
        count: 0,
        maxcount: MAX_POSITION_LEAFS as i32,
        // list: &mut leafs[0],
        // storeLeafs: CM_StoreLeafs,
        lastLeaf: 0,
        overflowed: false,
    };
    ...
    CM_BoxLeafnums_r(cm, &mut ll, 0, &mut |cm, ll: &mut leafList_t, nodenum: i32| {
        // was CM_StoreLeafs
        ...
        leafs[ll.count as usize] = leafNum;
        ll.count += 1;
    });
    ...
    for i in 0..ll.count as usize {
        CM_TestInLeaf(cm, tw, &cm.leafs[leafs[i] as usize]);
        if tw.trace.allsolid {
            break;
        }
    }
...
/*
fn CM_TraceThroughLeaf( tw: &mut traceWork_t, leaf: &cLeaf_t) {
    int         k;
    int         brushnum;
    cbrush_t    *b;
    cPatch_t    *patch;
...
================
*/
fn CM_TraceThroughSphere( tw: &mut traceWork_t,  origin: vec3_t,  radius: f32,  start: vec3_t,  end: vec3_t ) {
    f32 l1, l2, length, scale, fraction;
    f32 a, b, c, d, sqrtd;
...
================
*/
fn CM_TraceThroughVerticalCylinder( tw: &mut traceWork_t,  origin: vec3_t, radius: f32, halfheight: f32, start: vec3_t, end: vec3_t) {
    f32 length, scale, fraction, l1, l2;
    f32 a, b, c, d, sqrtd;
...
================
*/
fn CM_TraceCapsuleThroughCapsule( tw: &mut traceWork_t, model: clipHandle_t  ) {
    int i;
    vec3_t mins, maxs;
...
================
*/
fn CM_TraceBoundingBoxThroughCapsule( tw: &mut traceWork_t, model: clipHandle_t ) {
...
==================
*/
fn CM_TraceThroughTree( tw: &mut traceWork_t, num: usize,  p1f: f32,  p2f: f32,  p1: vec3_t,  p2: vec3_t) {
    cNode_t     *node;
    cplane_t    *plane;
    f32       t1, t2, offset;
    f32       frac, frac2;
...
==================
*/
fn CM_Trace( results: &mut trace_t, start: vec3_t, end: vec3_t, mins: &mut , maxs: &mut vec3_t,
                          model: clipHandle_t , origin: vec3_t, brushmask: i32, capsule: i32, sphere: Option<&sphere_t> ) {
    vec3_t      offset;

    let cmod = CM_ClipHandleToModel(model);
    ...
    let mut tw = traceWork_t::default();
...
==================
*/
fn CM_BoxTrace( results: &mut trace_t, start: vec3_t, end: vec3_t,
                          mins: &mut vec3_t,  maxs: &mut vec3_t,
                          model: clipHandle_t, brushmask: i32, capsule: i32 ) {
    CM_Trace( results, start, end, mins, maxs, model, vec3_origin, brushmask, capsule, None );
}
...
==================
*/
fn CM_TransformedBoxTrace( results: &mut trace_t, start: vec3_t, end: vec3_t,
                          mins: &mut vec3_t, maxs: &mut vec3_t,
                          model: clipHandle_t, brushmask: i32,
                          origin: vec3_t, angles: vec3_t, capsule: i32 ) {
    trace_t     trace;
